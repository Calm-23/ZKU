...
uint get_total_signal_no() {return 8888;}

uint get_number_of_components() {return 26;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 6605;}
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[9]); // line circom 44
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[3])) + 0);
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[3])) + 7)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[7]); // line circom 53
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[3];
// load src
...
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[3])) + 4)]].signalStart + 0]);
}
{
PFrElement aux_dest = &lvar[3];
// load src
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[9]); // line circom 74
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
