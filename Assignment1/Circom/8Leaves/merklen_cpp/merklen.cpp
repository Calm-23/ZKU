...
void LeafHash_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void LeafHash_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
void NonLeafHash_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void NonLeafHash_4_run(uint ctx_index,Circom_CalcWit* ctx);
void MerkleHelper_5_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MerkleHelper_5_run(uint ctx_index,Circom_CalcWit* ctx);
void MerkleN_6_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MerkleN_6_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 19548;}
...
ctx->componentMemory[coffset].templateName = "LeafHash";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "singleHash";
mySubcomponents[aux_create+i] = aux_cmp_num;
...
ctx->componentMemory[coffset].templateName = "NonLeafHash";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
ctx->componentMemory[coffset].templateName = "MerkleHelper";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 8; i++) {
std::string new_cmp_name = "leavesHash"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
LeafHash_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 888 ;
aux_cmp_num += 3;
}
}
...
uint csoffset = mySignalStart+7127;
uint aux_dimensions[1] = {7};
for (uint i = 0; i < 7; i++) {
std::string new_cmp_name = "nonLeavesHash"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
NonLeafHash_4_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
}
}
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[17]); // line circom 44
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[3])) + 0);
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[3])) + 15)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
LeafHash_2_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[3])) + 0)]].signalStart + 0]);
}
{
...
PFrElement aux_dest = &lvar[3];
// load src
Fr_add(&expaux[0],&lvar[3],&circuitConstants[3]); // line circom 44
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[17]); // line circom 44
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[15]); // line circom 53
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[3];
// load src
Fr_add(&expaux[0],&lvar[3],&circuitConstants[3]); // line circom 53
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[15]); // line circom 53
...
Fr_geq(&expaux[0],&lvar[3],&circuitConstants[1]); // line circom 57
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[3])) + 8);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * ((2 * Fr_toInt(&lvar[3])) + 1)) + 0)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
NonLeafHash_4_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * ((2 * Fr_toInt(&lvar[3])) + 2)) + 0)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
NonLeafHash_4_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[3])) + 8)]].signalStart + 0]);
}
{
PFrElement aux_dest = &lvar[3];
// load src
Fr_sub(&expaux[0],&lvar[3],&circuitConstants[3]); // line circom 57
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_geq(&expaux[0],&lvar[3],&circuitConstants[1]); // line circom 57
...
ctx->componentMemory[coffset].templateName = "MerkleN";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 8;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "hashedNodes";
mySubcomponents[aux_create+i] = aux_cmp_num;
MerkleHelper_5_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 19538 ;
aux_cmp_num += 53;
}
}
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[17]); // line circom 74
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + ((1 * Fr_toInt(&lvar[2])) + 15)];
// load src
// end load src
...
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
MerkleHelper_5_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &lvar[2];
// load src
Fr_add(&expaux[0],&lvar[2],&circuitConstants[3]); // line circom 74
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[17]); // line circom 74
...
MerkleN_6_create(1,0,ctx,"main",0);
MerkleN_6_run(0,ctx);
}
